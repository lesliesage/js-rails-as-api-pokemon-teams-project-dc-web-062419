...
### Adding a Pokemon
* Note: When adding a new pokemon, the nickname should be generated using `Faker::Name` from the `faker` gem and the
species should be generated using `Faker::Games::Pokemon`. See the `seeds.rb` file above as an example.
```text
#=> Example Request
POST /pokemons
